function gsp(fn) { return window.URL.createObjectURL(new Blob([(typeof fn === 'string' ? fn : fn.toString()).match(/^\s*function\s*\(\s*\)\s*\{(([\s\S](?!\}$))*[\s\S])/)[1]],{type: 'text/javascript'}));}
...
"dontInvert";break;case "invert":G="onlyInvert";break;case "both":G="attemptBoth";break;default:throw Error("Invalid inversion mode");}break;case "close":self.close()}}})(); });export default e
